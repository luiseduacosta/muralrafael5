...
BEGIN
    ...
    -- Check 1: Verify all referenced tables exist
    ...
    -- Check 2: Check for orphaned records in each foreign key relationship
    OPEN fk_cursor;
    ...
    read_loop: LOOP
        ...
        -- Check if tables exist before counting orphans
           ...
           AND EXISTS (SELECT 1 FROM information_schema.TABLES WHERE TABLE_NAME = referenced_table) THEN
            ...
            IF @orphan_count > 0 THEN
                ...
                VALUES ('ORPHAN_RECORDS',
                        CONCAT('Table `', table_name, '`.`', column_name, '` has ', @orphan_count, ' orphan records referencing `', referenced_table, '`'),
                        'WARNING');
            END IF;
        END IF;
    END LOOP;
    ...
    -- Check 3: Verify column data types match for foreign keys
    INSERT INTO validation_errors (error_type, error_message, severity)
...
-- Procedure to check if critical tables exist
CREATE PROCEDURE ValidateRequiredTables()
BEGIN
    DECLARE required_tables VARCHAR(255);
    
    -- Check for required tables before migration
...
-- Procedure to display validation results
...
SELECT COUNT(*) INTO @error_count FROM validation_errors WHERE severity = 'ERROR';
IF @error_count > 0 THEN
    SELECT '❌ Migration aborted due to validation errors. Please fix the issues and try again.' AS '';
    ROLLBACK;
...
-- =====================================================
-- PART 2: BACKUP EXISTING DATA (Optional but recommended)
-- =====================================================
...
-- Table USERS: Add new columns and change types
...
BEGIN
    ...
    SELECT CONCAT('✓ Foreign key constraints: ', foreign_key_count) AS '';
    
    -- Check for data loss in critical tables
    SELECT 
        'users' AS table_name,
...
SELECT '========================================' AS '';
SELECT 'All changes have been committed.' AS '';
SELECT 'Please review the validation results above.' AS '';
